...
public class Model {
	...
	public void startNewGame() {
		Random t = new Random();
		String f = _seedWords.get(t.nextInt(_seedWords.size()));
		generateWordsToFind(f);
		_inventoryLetters = string2charList(f);
		shuffle();
	...
	public void clear() {
		...
		for(Character q:_guessLetters) {
			p.add(q);
		}
		_guessLetters.removeAll(_guessLetters);
		for(int i = 0;i<p.size();i++) {
			_inventoryLetters.add(p.get(i));
		}
		notifyObservers();
		p.removeAll(p);
	}
	 ...
	 */
	public void submit() {
		if(_wordsToFind.containsKey(charList2string(_guessLetters))) {
			_wordsToFind.replace(charList2string(_guessLetters), true);
			r =1;
	...
	public ArrayList<String> words() {
		ArrayList<String> r = new ArrayList<>(_wordsToFind.keySet());
		ArrayList<String> d = new ArrayList<>();
		Collections.sort(r);
		for(int i = MINIMUM_WORD_LENGTH;i<=MAXIMUM_WORD_LENGTH;i++) {
			for(String g:filterWordsForLength(r,i)) {
				if(_wordsToFind.get(g)==false) {
					d.add("-------".substring(0, i));
				}
				if(_wordsToFind.get(g)==true) {
					d.add(g);
				}
			}
	...
	public String charList2string(ArrayList<Character> list) {
		StringBuilder create = new StringBuilder(list.size());
